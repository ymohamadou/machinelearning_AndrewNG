...
%Transform y to matrix before computing cost
Y = diag(ones(1,num_labels), 0);

for i=1:m  
  J = J + (-log(hypothesis(i,:))*Y(:, y(i)) - log(1-hypothesis(i,:))*(1-Y(:, y(i))));  
end
J = J/m;
...
for i=1:m
  ...
  %Compute errors
  smallDelta3 = a3(i,:)' - Y(:, y(i));
  smallDelta2 = Theta2'*smallDelta3.*sigmoidGradient([ones(1,1) z2(i,:)]');
  smallDelta2 = smallDelta2(2:end);
    
  delta1 = delta1 + smallDelta2*a1(i,:);
  delta2 = delta2 + smallDelta3*a2(i,:);  
end
